...
import siguard.laser.ethereum.util as util
from siguard.laser.ethereum import natives
from siguard.laser.ethereum.instruction_data import calculate_native_gas
from siguard.laser.ethereum.state.account import Account
from siguard.laser.ethereum.natives import PRECOMPILE_COUNT, PRECOMPILE_FUNCTIONS
from siguard.laser.ethereum.state.calldata import (
    BaseCalldata,
    SymbolicCalldata,
    ConcreteCalldata,
)
from siguard.laser.ethereum.state.global_state import GlobalState
from siguard.laser.smt import BitVec, If
from siguard.laser.smt import simplify, Expression, symbol_factory
...
SYMBOLIC_CALLDATA_SIZE = 512 # Used when copying symbolic calldata

class EcrecoverReturnValue(StateAnnotation):
    def __init__(self,value) -> None:
        super().__init__()
        self.value=value
        return
    def __copy__(self):
        new=EcrecoverReturnValue(self.value)
        return new
...
def native_call(
    ...
    mem_out_start = memory_out_offset
    ...
    global_state.mstate.min_gas_used += 100
    global_state.mstate.max_gas_used += 1000
    global_state.mstate.mem_extend(mem_out_start, mem_out_sz)

    try:
        data = natives.native_contracts(call_address_int, call_data)
    except natives.NativeContractException:
        log.debug("Failed to compute concrete value")
        for i in range(mem_out_sz):
            x = global_state.new_bitvec(
        ...
        if call_address_int==1:
            x.annotate(EcrecoverReturnValue(x))
            global_state.annotate(EcrecoverReturnValue(x))
        global_state.mstate.memory[mem_out_start + i] = x 
