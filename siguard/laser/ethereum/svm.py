...
from siguard.support.opcodes import OPCODES
from siguard.analysis.potential_issues import check_potential_issues
from siguard.laser.execution_info import ExecutionInfo
from siguard.laser.ethereum.cfg import NodeFlags, Node, Edge, JumpType
from siguard.laser.ethereum.evm_exceptions import StackUnderflowException, VmException
from siguard.laser.ethereum.instructions import Instruction
from siguard.laser.ethereum.instruction_data import get_required_stack_elements
from siguard.laser.plugin.signals import PluginSkipWorldState, PluginSkipState
...
from siguard.laser.ethereum.strategy.basic import DepthFirstSearchStrategy
from abc import ABCMeta
from siguard.laser.ethereum.time_handler import time_handler
...
class LaserEVM:
    ...
    def sym_exec(
        ...
        log.info("Starting LASER execution")
    ...
    def exec(self, create=False, track_gas=False) -> Optional[List[GlobalState]]:
        ...
        for global_state in self.strategy:
            ...
            if create and self._check_create_termination():
                log.info("Hit create timeout, returning.")
                return final_states + [global_state] if track_gas else None

            if not create and self._check_execution_termination():
                log.info("Hit execution timeout, returning.")
                return final_states + [global_state] if track_gas else None
            try:
                new_states, op_code = self.execute_state(global_state)
            except NotImplementedError:
                log.info("Encountered unimplemented instruction")
                continue
    ...
    def execute_state(
        ...
        try:
            ...
            log.info("PluginSkipState: Continue!")
            return [], None
        ...
        try:
            ...
            log.info("_execute_pre_hook: PluginSkip!")
            return [], None
        ...
        try:
            ...
            if len(new_global_states)==0:
                log.info("Error!!!")
                log.info(op_code)

        except VmException as e:
            log.info("get VmException!")
            ...
            log.info("Starting new transaction %s", start_signal.transaction)
            ...
            if return_global_state is None:
                if (
                    ...
                    log.info("Check_Potential_Issues Finished!")
        ...
        for hook in self.post_hooks[op_code]:
            for global_state in global_states:
                try:
                    ...
                    log.info("SKIP Global State Due to Hook PluginSkipState")
