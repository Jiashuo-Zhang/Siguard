...
from siguard.laser.ethereum.util import getHashsInMem, isCalldataVar,hasAddressVar
...
from siguard.laser.ethereum.util import isSloadVar, isSstoreVar
...
class Memory:
    ...
    def write_word_at(self, index: Union[int,BitVec], value: Union[int, BitVec, bool, Bool]) -> None:
        ...
        try:
            ...
            elif isinstance(value,BitVec) and hasAddressVar(value):
                for i in range(0, 256, 8):
                    self[index + 31 - (i // 8)] = Extract(i + 7, i, value)
                assert value.size()==256
                logging.debug("Write a Address Var to Memory")
            elif isinstance(value,BitVec) and isSloadVar(value):
                for i in range(0, 256, 8):
                    self[index + 31 - (i // 8)] = Extract(i + 7, i, value)
                assert value.size()==256
                logging.debug("Write a Sload Var to Memory")
            elif isinstance(value,BitVec) and isSstoreVar(value):
                for i in range(0, 256, 8):
                    self[index + 31 - (i // 8)] = Extract(i + 7, i, value)
                logging.debug("Write a sStore Var to Memory")
                assert value.size()==256
            elif isinstance(value,BitVec) and isCalldataVar(value):
                for i in range(0, 256, 8):
                    self[index + 31 - (i // 8)] = Extract(i + 7, i, value)
                logging.debug("Write a Calldata Var to Memory")
                assert value.size()==256
            elif isinstance(value,BitVec) and len(getHashsInMem([value]))>=1:
                for i in range(0, 256, 8):
                    self[index + 31 - (i // 8)] = Extract(i + 7, i, value)
                logging.debug("Write a Hash Memory Var to Memory")
                assert value.size()==256
            else:
    ...
    def __getitem__(
        ...
        if isinstance(item, slice):
            ...
            if start is None:
                logging.debug("Memory Start is None!!")
                start = 0
