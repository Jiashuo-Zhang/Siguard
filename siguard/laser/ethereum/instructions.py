...
from siguard.laser.smt import symbol_factory

from siguard.disassembler.disassembly import Disassembly

from siguard.laser.ethereum.state.calldata import ConcreteCalldata, SymbolicCalldata

import siguard.laser.ethereum.util as helper
from siguard.laser.ethereum import util
from siguard.laser.ethereum.function_managers import (
    keccak_function_manager,
    exponent_function_manager,
)

from siguard.laser.ethereum.call import (
    get_call_parameters,
    native_call,
    get_call_data,
    SYMBOLIC_CALLDATA_SIZE,
)
from siguard.laser.ethereum.evm_exceptions import (
    VmException,
...
from siguard.laser.ethereum.instruction_data import get_opcode_gas, calculate_sha3_gas
from siguard.laser.ethereum.state.global_state import GlobalState
from siguard.laser.ethereum.state.return_data import ReturnData

from siguard.laser.ethereum.transaction import (
...
from siguard.support.support_utils import get_code_hash

from siguard.support.loader import DynLoader
from siguard.laser.ethereum.state.annotation import StateAnnotation

from siguard.laser.ethereum.util import hasAddressVar, CopyAllAnnotation,isHashVar,isSloadVar,sloadAnnotions,getStorage,getPotentialNonce,hasAddress,getHashsInMem
...
class StorageEIP712SeperatorAnnotation(StateAnnotation):
    def __init__(self,slot:int) -> None:
        super().__init__()
        self.slot=slot
        return
    def __copy__(self):
        new=StorageEIP712SeperatorAnnotation(self.slot)
        return new
...
class SLoadAnnotation(StateAnnotation):
    def __init__(self,slot,value) -> None:
        self.slot=slot
        self.value=value
    def __copy__(self):
        new_annotation = SLoadAnnotation(
            self.slot,self.value
        )
        return new_annotation

class MemoryStoreWordAnnotation(StateAnnotation):
    def __init__(self,slot,value) -> None:
        self.slot=slot
        self.value=value
    def __copy__(self):
        new_annotation = MemoryStoreWordAnnotation(
            self.slot,self.value
        )
        return new_annotation


class ContractAddressAnnotation(StateAnnotation):
    def __init__(self,address:BitVec) -> None:
        self.address=address
        return
    def __copy__(self):
        new_annotation = ContractAddressAnnotation(
            self.address
        )
        return new_annotation
    # def persist_over_calls(self) -> bool:
    #     return True
class SStoreAnnotation(StateAnnotation):
    def __init__(self,slot,value) -> None:
        self.slot=slot
        self.value=value
    def __copy__(self):
        new_annotation = SStoreAnnotation(
            self.slot,self.value
        )
        return new_annotation   
    # def persist_over_calls(self) -> bool:
...
class CallDataAnnotation(StateAnnotation):
    def __init__(self,slot,size,value) -> None:
        self.slot=slot
        self.size=size
        self.value=value
    def __copy__(self):
        new_annotation = CallDataAnnotation(
            self.slot,self.size,self.value
        )
        return new_annotation


class HashAnnotation(StateAnnotation):
    def __init__(self, hashVariable: BitVec, data:Union[int, BitVec],hasNonce:bool=False,hasAddress:bool=False) -> None:
        self.hashVariable=hashVariable
        self.data=data
        self.nonce=hasNonce
        self.address=hasAddress

    def __copy__(self):
        new_annotation = HashAnnotation(
            self.hashVariable,self.data
        )
        return new_annotation
    def setNonce(self):
        self.nonce=True
    def hasNonce(self) -> bool:
        return self.nonce
    def setAddress(self):
        self.address=True
...
class Instruction:
    ...
    def calldataload_(self, global_state: GlobalState) -> List[GlobalState]:
        ...
        environment = global_state.environment
        op0 = state.stack.pop()
        log.debug("Calldata Load")
        value = environment.calldata.get_word_at(op0)
        anno = CallDataAnnotation(op0,32,value)
        value.annotate(anno)
        global_state.annotate(anno)
    ...
    def _calldata_copy_helper(global_state, mstate, mstart, dstart, size):
        ...
        try:
            ...
            log.info("Unsupported symbolic size in CALLDATACOPY")
        ...
        if size > 0:
            try:
                ...
                log.warning("Memory allocation error: {}".format(e))
            ...
            try:
                ...
                for i in range(size):
                    ...
                    anno=CallDataAnnotation(i,1,value)
                    value.annotate(anno)
                    global_state.annotate(anno)
                ...
                log.info("Exception copying calldata to memory")
    ...
    def address_(self, global_state: GlobalState) -> List[GlobalState]:
        ...
        :return:
        """
        log.debug("MEET ADDRESS!!!")
        state = global_state.mstate
        environment = global_state.environment
        x=environment.address
        x.annotate(ContractAddressAnnotation(x))
    ...
    def caller_(self, global_state: GlobalState) -> List[GlobalState]:
        ...
        caller.annotate(CallDataAnnotation(0,1,caller))
        state.stack.append(caller)
        return [global_state]
    ...
    def sha3_(self, global_state: GlobalState) -> List[GlobalState]:
        ...
        try:
            ...
            # Can't access symbolic memory offsets # TODO
            log.info("Can't access symbolic memory offsets, use 128 to replace it.")
            length = 128
            global_state.world_state.constraints.append(op1 >= 20)
        ...
        hasaddress=hasAddress(data_list)
        if hasaddress==True:
            result.annotate(ContractAddressAnnotation(BitVec(0)))
            global_state.world_state.annotate(StorageEIP712SeperatorAnnotation(0))
        for i in CopyAllAnnotation([data]):
            result.annotate(i)
        

        ha=HashAnnotation(result,data,hasAddress=hasaddress)

        result.annotate(ha)

        state.stack.append(result)
        global_state.add_annotations([ha])
    ...
    def extcodesize_(self, global_state: GlobalState) -> List[GlobalState]:
        ...
        try:
            ...
            log.info("error accessing contract storage due to: " + str(e))
            state.stack.append(global_state.new_bitvec("extcodesize_" + str(addr), 256))
            return [global_state]
    ...
    def returndatasize_(self, global_state: GlobalState) -> List[GlobalState]:
        ...
        if global_state.last_return_data:
            ...
            global_state.mstate.stack.append(0) #TODO
        return [global_state]
    ...
    def mstore_(self, global_state: GlobalState) -> List[GlobalState]:
        ...
        try:
            ...
            log.info("Error extending memory")
        log.debug(getHashsInMem([value]))
        state.memory.write_word_at(mstart, value)
        global_state.annotate(MemoryStoreWordAnnotation(mstart,value))
    ...
    def sload_(self, global_state: GlobalState) -> List[GlobalState]:
        ...
        x=global_state.environment.active_account.storage[index]
        log.debug(hasAddressVar(x))
        anno=SLoadAnnotation(index,x)
        x.annotate(anno)
        global_state.annotate(anno)
        state.stack.append(x)
        for i in sloadAnnotions(x):
            global_state.annotate(i)
    ...
    def sstore_(self, global_state: GlobalState) -> List[GlobalState]:
        ...
        global_state.annotate(SStoreAnnotation(index,value))
        if hasAddressVar(value) and isHashVar(value):
            log.debug("Find EIP 712 Seperator!!!")
            global_state.world_state.annotate(StorageEIP712SeperatorAnnotation(index))
    ...
    def jumpi_(self, global_state: GlobalState) -> List[GlobalState]:
        ...
        :return:
        """
        log.debug("Evaluating Jumpi and Choose Next States")
        state = global_state.mstate
        disassembly = global_state.environment.code
        ...
        try:
            ...
            log.info("Skipping JUMPI to invalid destination.")
        ...
        if negated_cond:
            ...
            log.debug("Pruned unreachable states: Always True")
            log.debug(condition)
        ...
        if index is None:
            log.info("Invalid jump destination: " + str(jump_addr))
            return states
        ...
        if instr["opcode"] == "JUMPDEST":
            if positive_cond:
                ...
                log.debug("Pruned unreachable states: Always False")
                log.debug(condition)
        return states
    ...
    def staticcall_(self, global_state: GlobalState) -> List[GlobalState]:
        ...
        try:
            ...
            if callee_account is not None and callee_account.code.bytecode == "":
                log.info("The call is related to ether transfer between accounts")
                sender = environment.active_account.address
                receiver = callee_account.address
                transfer_ether(global_state, sender, receiver, value)
                self._write_symbolic_returndata(
        ...
        if native_result:
            log.debug("Nativa Call, Return Global State")
            return native_result
    ...
    def post_handler(self, global_state, function_name: str):
        ...
        if function_name in ("staticcall", "delegatecall"):
            log.debug("Enter Post Handler for Staticcall!!")
            memory_out_size, memory_out_offset = global_state.mstate.stack[-6:-4]
        else:
            memory_out_size, memory_out_offset = global_state.mstate.stack[-7:-5]
